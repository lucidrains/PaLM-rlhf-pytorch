...
palm = PaLM(num_tokens=20000, dim=256, depth=4).to(device)


# load your pretrained reward model

reward_model = RewardModel(palm, num_binned_output=5).to(device)
...
prompt_mask = (
    torch.zeros(1, 1024).bool().to(device)
...
trainer = RLHFTrainer(palm=palm, reward_model=reward_model, prompt_token_ids=prompts)
